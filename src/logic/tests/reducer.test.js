...
import { addItem, deleteItem, complete, inComplete } from '../actions';
...
describe('reducer', () => {
  ...
  it('should remove items on DELETE_ITEM', () => {
    const mockAction = deleteItem(2);
    const result = reducer(undefined, mockAction);
    expect(result.items).toHaveLength(1);
  });

  it('should set completed to true for given task id', () => {
    const mockAction = complete(2);
    const result = reducer(undefined, mockAction);
    expect(result.items[1].completed).toBe(true);
  });

  it('should set completed to false for given task id', () => {
    const mockAction = inComplete(2);
    const result = reducer(undefined, mockAction);
